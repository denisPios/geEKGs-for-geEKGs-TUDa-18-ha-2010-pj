...
def get_feature(ecg_leads, fs):
    ...
    for idx, ecg_lead in enumerate(ecg_leads):
        ...
        try:
            _, r_peaks = nk.ecg_peaks(ecg_lead, fs)
            r_peaks=pd.Series(r_peaks['ECG_R_Peaks'])
        except:
            #does nothing
            e=0
        if(len(r_peaks)==0):
        ...
        # I got twice a error sayign it wasnt possible to do the dataframe, because the length of the list were not the same, no idea why
            data = {'mean_nni': mean_nni, 'sdnn': sdnn, 'sdsd': sdsd, 'nni_50': nni_50, 'pnni_50': pnni_50, 'nni_20': nni_20, 'pnni_20': pnni_20, 'rmssd': 
 rmssd, 'median_nni': median_nni, 'range_nni': range_nni, 'cvsd': cvsd, 
'cvnni': cvnni, 'mean_hr': mean_hr, 'max_hr': max_hr, 
 'min_hr': min_hr, 'std_hr': std_hr,                   #'total_power':total_power,'vlf':vlf,'lf':lf,'hf':hf,'lf_hf_ratio':lf_hf_ratio,
'csi':csi,'cvi':cvi}
