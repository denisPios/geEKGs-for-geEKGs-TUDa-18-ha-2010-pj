...
def mean_RR_interval(r_peaks):
    ...
    if r_peaks == 1 or r_peaks == 0:
        return 0
    for i in range(0, len(r_peaks) - 1):
        RR = np.append(RR, r_peaks[i+1] - r_peaks[i])
    return RR
...
def get_feature(ecg_leads, fs):
    ...
    for idx, ecg_lead in enumerate(ecg_leads):
        ...
        try:
            ...
            print('???')
            e=0
        if(len(r_peaks)==0 or len(r_peaks)==1 or len(r_peaks)==2):
            print('we got a none r_peak signal!!! which is {}'.format(idx))
            mean_nni.append(-1)
            sdnn.append(-1)
            sdsd.append(-1)
            nni_50.append(-1)
            ...
            if(len(r_peaks) == 1):               
                ...
                #r_peaks.append(r_peaks[0])
                print('only one r_peak signal!!!which is {}'.format(idx))
                #R peak locations in ms
            #r_peaks = np.array(r_peaks)/fs*1000            
            r_peaks = mean_RR_interval(r_peaks)
    ...
    data = {'mean_nni': mean_nni, 'sdnn': sdnn, 'sdsd': sdsd, 'nni_50': nni_50, 'pnni_50': pnni_50, 'nni_20': nni_20, 'pnni_20': pnni_20, 'rmssd':  rmssd, 'median_nni': median_nni, 'range_nni': range_nni, 'cvsd': cvsd, 'cvnni': cvnni, 'mean_hr': mean_hr, 'max_hr': max_hr, 'min_hr': min_hr, 'std_hr': std_hr, 'csi':csi,'cvi':cvi}
